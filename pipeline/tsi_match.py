...
def compute_tsi_modifiers(
    alert,
    bovada_entry,
    projections: list[TSIProjection],
    bets: list[TSIBet],
) -> tuple[float, float, bool]:
    """
    Compute TSI-based confidence modifiers for a play.

    Returns (tsi_edge, alignment_multiplier, is_tsi_bet).
    """
    tsi_edge = 0.0
    alignment_mult = 1.0
    is_tsi_bet = False

    proj = _find_tsi_projection(alert, projections)

    if proj and bovada_entry:
        if alert.market == "Spread":
            # Get the team from the sharp side
            team_name = re.sub(r"\s+[+-][\d.]+$", "", alert.side).strip()
    ...
    # Check explicit TSI bets
    for bet in bets:
        if _is_tsi_bet_match(alert, bet):
            is_tsi_bet = True
            break

    return tsi_edge, alignment_mult, is_tsi_bet
